...
> [!WARNING]
> 本仓库提供的是教学实现，不是可直接用于生产的 Agent 代码。部分可运行章节为了直观展示 Agent Loop，刻意允许模型调用本机 Shell。请勿让它处理不可信提示词，也不要在高权限环境中运行。

- 使用一次性工作区、低权限账号和受限 API Key。
- 不要在示例中复用生产环境密钥。
- 执行前审查工具调用；真实部署必须增加沙箱、命令白名单和审批。
- 生产场景优先使用官方 [Hermes Agent](https://github.com/NousResearch/hermes-agent) 及其审批、权限与隔离机制。
